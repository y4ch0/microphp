...
TEXT)
            . <<<'HTML'
    <p>The skeleton maps <code>App\</code> to <code>app/</code>. Its <code>yacho/microphp-core</code> dependency maps <code>MicroPHP\</code> to the framework source under <code>vendor/</code> and loads the framework helper functions automatically.</p>
</section>
...
<section>
    <h1>HTTP Layer</h1>
    <p>The core package's <code>src/Http</code> directory contains dependency-free request and response objects. They are inspired by PSR-style HTTP objects but stay small enough to read in one sitting.</p>
</section>
...
JSON)
            . <<<'HTML'
    <p><code>type: project</code> is appropriate for an application skeleton. Framework classes and helpers come from the required <code>yacho/microphp-core</code> library; this package owns application autoloading, CLI binaries, and project setup.</p>
</section>
...
<section>
    ...
    <p>Publish and tag the core library first. Then update and commit the skeleton lock file, validate the complete application, tag the skeleton, and submit its public Git URL to Packagist.</p>
</section>
...
<section>
    ...
    <p>Application features belong in <code>app</code>, component folders, guards, API handlers, middleware, and services. Change framework internals in the separate <code>yacho/microphp-core</code> repository only when framework behavior itself needs to evolve, and cover those changes with core tests.</p>
</section>
